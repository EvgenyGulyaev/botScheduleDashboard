...
func PostChatCallStart(ctx *silverlining.Context, conversationID string) {
	...
	call, _, _, err := store.GetChatRepository().StartCall(conversationID, user.Email, user.Login)
	if err != nil {
	...
	if err == nil {
		...
		if messageErr == nil {
			if publishErr := producer.PublishChatCallStartedEvent(event.ChatCallStartedEvent{
